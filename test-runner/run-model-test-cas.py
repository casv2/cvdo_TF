...
#args.element = args.label

dir_name = '../test-results/{0}/model-{1}-test-{2}'.format(args.element, args.model_name, args.test_name)
if not os.path.exists(dir_name) and __builtin__.do_io:
    os.mkdir(dir_name)
time.sleep(1)
os.chdir(dir_name)

json_file_name = os.path.join('..', 'element-{0}-model-{1}-test-{2}-properties.json'.format(args.element, args.model_name, args.test_name))
...
model_dir = os.path.join('..', '..', '..', 'models', args.element, args.model_name)
test_dir = os.path.join('..', '..', '..', 'tests', args.element, args.test_name)
...
if args.redirect_stdout:
    ...
    if __builtin__.do_io:
        log = open('../element-{0}-model-{1}-test-{2}.txt'.format(args.element, args.model_name, args.test_name), 'w', 0)
